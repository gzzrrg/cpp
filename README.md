...
~~~c++
for(string::iterator t = a1.begin();t < a1.end();t++){
        cout<< *t ;
    }
~~~
...
- 名称[n] --支持下标访问，类似数组
- empty --返回是否为空 (T/F)
- begin / end --迭代器，遍历使用，可以++或--。左闭右开，与地址绑定。
...
- size/empty  --返回队列实际元素个数/返回队列是否为空
- push/pop --入队/出队
- front/back --返回队头元素/返回队尾元素 （不删除）
...
int main() {
    ...
    lb = y.size();
    lc = max(la,lb);
    for (int i = 0; i < la; ++i) arr1[la-1-i] = x[i] - '0';
    for (int i = 0; i < lb; ++i) arr2[lb-1-i] = y[i] - '0';
    for (int i = 0; i < lc; ++i) {
        arr1[i] += arr2[i];
        if (arr1[i] >9){
            arr1[i+1] ++;
...
int main() {
    ...
    string x,y;  cin >> x >> y;
//  字符串长度 （x < y）？ 加上负号，交换字符串
    if (x.size() < y.size() || (x<y && x.size() == y.size())){
        cout << '-';
        swap(x,y);
    } else if (x.size() == y.size() && x == y){ //x.size==y.size && x==y
        cout << '0';
        return 0;
    }
...
int arr1[N],arr2[N],arr3[N],la,lb,lc;

void num(int array1[],int array2[],int array3[]){
    for (int i = 0; i < la; ++i) {
        for (int j = 0; j < lb; ++j) {
...
int main() {
    ...
    la = (int) x.size();
    lb = (int) y.size();
    lc = la +lb; //最后输出位数，没处理前导0

    for (int i = 0; i < la; ++i) arr1[i] = x[la-1-i] - '0';
    for (int i = 0; i < lb; ++i) arr2[i] = y[lb-1-i] - '0';

    num(arr1,arr2,arr3);

    for (int i = lc; i >= 0; --i) cout << arr3[i];
    return 0;
}
~~~
...
int main() {
    ...
    //处理前导0
    while (la > 0 && b[la] == 0) la--;
    for (int i = la; i >= 0; --i) cout << b[i];
...
int main() {
    ...
    //取出数据到tt，个位 -> 十位，进行除c放入b[]，对c取模放回tt。
    long long tt = 0;
    for (int i = la-1; i >= 0; --i) {
        tt = tt*10 + a[i];
        b[i] =  tt/c;
